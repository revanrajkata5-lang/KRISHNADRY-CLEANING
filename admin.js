// TODO: Replace with YOUR Firebase project's config — must match store.js exactly.
...
// TODO: Replace with YOUR Supabase project's URL and anon key — must match store.js exactly.
...
function buildWhatsAppBillText(order) {
  ...
  const lines = [];
  lines.push("🧾 *Krishna Dry Cleaning and Washing Shop*");
  if (currentDlNumber) lines.push(`License No: ${currentDlNumber}`);
  lines.push("");
...
function buildReceiptHTML(order) {
  ...
  const dlLine = currentDlNumber
    ? `<p class="center meta">License No: ${escapeHtml(currentDlNumber)}</p>`
    : "";
...
<body>
  ...
  <h2 class="center">KRISHNA DRY CLEANING</h2>
...
async function loadSiteLiveState() {
  const { data, error } = await supabaseClient
    .from("store_settings")
    .select("is_live, dl_number, admin_whatsapp")
    .eq("id", 1)
    .single();
  if (error) {
    console.error("store_settings load failed:", error.message);
    showBackendWarning('Site-live toggle needs a "store_settings" table (id, is_live, dl_number, admin_whatsapp) in Supabase — see admin.js for the create-table snippet.');
    return;
  }
  applyLiveState(!!data.is_live);
  currentDlNumber = data.dl_number || "";
  applyDlNumber(currentDlNumber);
  currentAdminWhatsApp = data.admin_whatsapp || "";
  applyAdminWhatsApp(currentAdminWhatsApp);
}
  ...
  adminWhatsAppSaveBtn.addEventListener("click", async () => {
    ...
    if (adminWhatsAppStatus) {
      ...
      if (error) {
        ...
        setTimeout(() => {
          adminWhatsAppStatus.hidden = true;
        }, 2500);
      }
    }
  });
